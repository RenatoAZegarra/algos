 ...
 */
function generateAnagrams(str, startIndex = 0, anagrams = []) {
  if (startIndex === str.length - 1) {
    // Base case: no more characters to swap, add permutation to anagrams array
    anagrams.push(str);
  } else {
    // Recursive case: generate all possible permutations by swapping characters
    for (let i = startIndex; i < str.length; i++) {
      const strArr = str.split("");
      [strArr[startIndex], strArr[i]] = [strArr[i], strArr[startIndex]]; // swap characters
      generateAnagrams(strArr.join(""), startIndex + 1, anagrams); // generate permutations of remaining characters
    }
  }
  return anagrams;
}
const actual1 = generateAnagrams(str1).sort();
console.log(actual1);
 ...
 */
function lowestCommonMultiple(a, b, product = a) {
  if (a === 0 || b === 0) {
    return 0;
  }
  if (a === b) {
